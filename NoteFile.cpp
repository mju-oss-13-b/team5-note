	...
	: m_pFile(NULL)
{
	if ( sizeof( m_head ) > START_POS ){
		cout << "-- 심각한 에러: 데이터가 덮어씌워진 것 같습니다! --" << endl;
	}
...
bool CNoteFile::Open()
{
	...
	if ( m_pFile ){
		...
		if ( IsVaild() ){
			...
			cout << "-- 에러: 노트파일의 형식이 유효하지 않습니다! --" << endl;
		}
	}else {
		cout << "-- 에러: 노트를 여는데 실패하였습니다! --" << endl;
	}
	return false;
}
...
void CNoteFile::Lock( bool isForever )
{
	if (isForever){
		...
		m_head.lastLock = time(NULL) + (60*60*24);	// ��24Сʱ
	}
}
...
{
	CEncrypt::Right( m_head.pwd, PWD_LEN, &m_head.key, 1 );	// ����
